...
The previous attempts kept failing because file contents were cross-contaminated: `go.mod`/`go.sum` contained Go source code, while `main.go`, `proxy.go`, and `config_json.go` were missing their `package main` declarations/imports and were truncated. This response:

- Restores a minimal, valid `go.mod` (module + go directive only, no `toolchain` line, no stray text) and an **empty `go.sum`**.
- Removes all third-party dependencies (`gopkg.in/yaml.v3`, `prometheus/client_golang`) by replacing Prometheus metrics with a small dependency-free `metrics.go`, so `go.sum` can legitimately be empty — eliminating "missing go.sum entry" failures entirely.
- Fully reconstructs `main.go`, `proxy.go`, and `config_json.go` as complete, compilable files with proper `package main` headers, imports, and no truncation, including previously-missing symbols (`NewMetrics`, `NewProxyServer`, `NewMux`, `buildTLSListenerConfig`, `gracefulShutdown`, `envOr`) that `main.go` depends on.
